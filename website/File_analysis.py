...
def cleanup_expired_sessions():
    ...
    file_in_specific_path = os.path.join(
        specific_path, os.path.basename(filepath))
...
def fileanalysis():
    ...
    if request.method == 'POST':
        input_method = request.form.get(
            'input-method') or request.json.get('input_method')
        ...
        if input_method == 'text':
            ...
            if not example_file:
                ...
                current_app.logger.error(
                    "The example_file is not provided or is None")
                raise ValueError("The example_file is not provided or is None")

            file_path = os.path.join(
                '/freetxt/website/static/example-data-hub', example_file)
                ...
                if file_extension == '.csv':
                    ...
                    try:
                        ...
                        try:
                            ...
                            data = pd.read_csv(
                                file_path, delimiter='\t', encoding='ISO-8859-1')
                        except:
                            # As a last resort, replace invalid characters
                            data = pd.read_csv(
                                file_path, delimiter='\t', encoding='utf-8', errors='replace')
            ...
            # Convert the DataFrame to JSON and store in session
            ...
            if file and allowed_file(file.filename):
                ...
                # save the new file path to session
                session['uploaded_file_path'] = filepath
                session['uploaded_file_name'] = filename
                data = read_file(filepath)
                # Convert the DataFrame to JSON and store in new session
                session['data'] = data.to_json()
                return jsonify({"columns": list(data.columns)})
            ...
            data = convert_date_columns(data, date_formats)
            extracted_data = data[selected_columns].dropna(
...
def handle_selected_sentences():
    ...
    for sentence in selected_sentences:
        ...
        filtered_words = [
            word for word in extracted_words if word not in STOPWORDS and word not in PUNCS]
    ...
    sentiment_data, sentiment_counts, pie_chart_html, bar_chart_html = sentiment_analysis(
        selected_sentences, language)
    df_sentiment = pd.DataFrame(sentiment_data)
    with file_lock:
        scatter_text_html = sentiment_analyser.generate_scattertext_visualization(
            df_sentiment, language)
    # Word Tree Generator
...
def sentiment_analysis(sentences, language, sentiment_classes=3):
    ...
    results = analyser.analyse_sentiment(
        sentences, language, sentiment_classes)
    if results:
        sentiments, sentiment_counts = results
        dfanalysis = pd.DataFrame(
            sentiments, columns=['Review', 'Sentiment Label', 'Confidence Score'])
        ...
        fig.write_image(
            "/freetxt/website/static/Sentiment_plots/sentiment_pie.png")
        fig.write_html(
            "/freetxt/website/static/Sentiment_plots/sentiment_pie.html")
    ...
    """
        ...
        fig_bar.write_image(
            "/freetxt/website/static/Sentiment_plots/sentiment_bar.png")
       ...
       # append_logo_to_image(fig_bar_path, logo_path, fig_bar_path)
        fig_bar.write_html(
            "/freetxt/website/static/Sentiment_plots/sentiment_bar.html")
...
def handle_sentiment_update():
    ...
    sentiment_data, sentiment_counts, pie_chart_html, bar_chart_html = sentiment_analysis(
        sentences, language, chosen_classes)
...
# print(cloud_shape_path)
        ...
        wc_path, word_list = cloud_generator.generate_wordcloud(
            cloud_shape_path, cloud_outline_color, cloud_type, language, cloud_measure, wordlist={})
        ...
        if cloud_type == 'semantic_tags' and session["tokens_with_semantic_tags"]:
            ...
            sec_wc_path, sec_word_list = cloud_generator.generate_wordcloud(
                cloud_shape_path, cloud_outline_color, 'all_words', language, cloud_measure, words_with_sem_tags, f"{time.time()}_words")

            # Cleans word list in case of values that can cause errors
            sec_word_list = [
                x for x in sec_word_list if str(x).lower() != 'nan']

            tag_words_associations = {tag: list({str(word) for (word, pos, tag_entry) in words_tags if tag == tag_entry}) for (
...
def analyse():
    ...
    try:
        ...
        if kwic_option == "word" and selected_word:
            ...
            collocs_strengths = analyzer.get_collocation_strength(
                selected_word)
        elif kwic_option == "tag" and selected_tag:
            kwic_results = analyzer.get_kwic(
                selected_tag, window_size, by_tag=True)
            if not kwic_results:
                ...
                collocs_strengths = analyzer.get_collocation_strength(
                    selected_word if selected_word else selected_tag, by_tag=True)

        elif kwic_option == "sem" and selected_semantic:
            kwic_results = analyzer.get_kwic(
                selected_semantic, window_size, by_sem=True)
            collocs_strengths = analyzer.get_collocation_strength(
                selected_word if selected_word else selected_semantic, by_sem=True)
        else:
            return jsonify({"error": "Invalid KWIC option or missing selection!"})

        collocs = analyzer.get_collocs(kwic_results)
        combined_collocs = [(colloc, freq, mi, ll) for (
            colloc, freq) in collocs for _, mi, ll in collocs_strengths if colloc == _]
        ...
        graph_path = analyzer.plot_coll_14(
            keyword_for_plot, collocs) if collocs and keyword_for_plot else None
...
def check_language():
    try:
        ...
        if has_english and not has_welsh:
            message = f'Your data is only in English from file: {
                original_file_name}' if language == 'en' else f'Mae eich data yn Saesneg yn unig o\'r ffeil: {original_file_name}.'
            return jsonify({'message': message})

        elif has_welsh and not has_english:
            message = f'Your data is only in Welsh from file: {
                original_file_name}' if language == 'en' else f'Mae eich data yn Gymraeg yn unigo\'r ffeil: {original_file_name}.'
            return jsonify({'message': message})
            ...
            english_path = os.path.abspath(os.path.join(
                '/freetxt/website/static/temp', english_filename))
            welsh_path = os.path.abspath(os.path.join(
                '/freetxt/website/static/temp', welsh_filename))

            pd.concat(english_data_dict.values()).to_csv(
                english_path, index=False)
...
def get_image_scatter():
    if 'scatter_html' in session:
        scatter_html = session.get('scatter_html')
        absolute_html_path = os.path.join(
            FileAnalysis.root_path, scatter_html.strip('/'))
        ...
        if os.path.exists(absolute_html_path):
            ...
            modified_html_path = os.path.join(
                '/freetxt/website/static/wordcloud', modified_html_filename)
            ...
            try:
                ...
                hti.screenshot(html_file=modified_html_path,
...
def add_logo_and_text_to_image(image_path):
    ...
    logo_image = Image.open(
        "home/khallafn/Freetxt-flask/website/static/images/logo.png")
    ...
    draw.text((120, wc_image.height - 45),
              visualisation_text, (0, 0, 0), font=font)
...
def get_word_cloudsrc():
    if 'word_cloud_src' in session:
        ...
        add_logo_and_text_to_image(os.path.join(
            '/freetxt/website/static/wordcloud/', filename))
    return send_from_directory(directory='static/wordcloud/', path=filename, as_attachment=True)
...
def get_sec_word_cloudsrc():
    if 'sec_word_cloud_src' in session:
        ...
        filename = os.path.basename(word_cloud_src)
        add_logo_and_text_to_image(os.path.join(
            'website/static/wordcloud/', filename))
    return send_from_directory(directory='static/wordcloud/', path=filename, as_attachment=True)
...
def get_filtered_word_frequencies():
    try:
        if 'word_frequencies' in session:
            ...
            # Sorted by frequency
            sorted_word_frequencies = {k: v for k, v in sorted(
                word_frequencies.items(), key=lambda item: (-item[1], item[0]))}
...
def get_unfiltered_word_frequencies():
    try:
        if 'unfiltered_word_frequencies' in session:
            ...
            # Sorted by frequency
            sorted_unfiltered_word_frequencies = {k: v for k, v in sorted(
                unfiltered_word_frequencies.items(), key=lambda item: (-item[1], item[0]))}
...
def retain_clusters(text, clusters):
    words = text.split()
    retained_words = [word for word in words if any(
        cluster in word for cluster in clusters)]
