...
path_to_model = Path(
    '/freetxt/venv/lib/python3.10/site-packages/en_dual_none_contextual/en_dual_none_contextual-0.3.1')
# stopwords_files
...
class WordCloudGenerator:
    ...
    def __init__(self, input_data):
        ...
        self.pymusaslist = pd.read_csv(
            '/freetxt/website/data/Pymusas-list.txt', names=['USAS Tags', 'Equivalent Tag'])
        self.text = ''
    ...
    def Pymsas_tags(self, text):
        ...
        if lang_detected == 'cy':
            ...
            merged_df = pd.merge(cy_tagged, self.pymusaslist,
            ...
            tags_to_remove = ['Unmatched',
                              'Grammatical bin', 'Pronouns', 'Period']
            merged_df = merged_df[~merged_df['USAS Tags'].str.contains(
                '|'.join(tags_to_remove))]
    ...
    def get_wordcloud(self, dataframe, metric, word_list, cloud_shape_path, cloud_outline_color, cloud_type, unique_id=None):
        ...
        if metric == 'Frequency':
            try:
                ...
                frequency_dist = dict(
                    zip(filtered_df['word'], filtered_df['freq']))
                frequency_dist = {word: math.log(
                    freq + 1) for word, freq in frequency_dist.items()}
       ...
       # scaled_frequencies = {word: self.combined_scaling(freq, max_freq)
        ...
        try:
            top_10_words = dict(
                sorted(frequency_dist.items(), key=lambda item: item[1], reverse=True)[:10])
...
# Apply combined scaling to the top 10 words
            scaled_top_words = {word: self.combined_scaling(
                freq, max_freq_top) for word, freq in top_10_words.items()}
...
# Apply a different scaling to the rest of the words
            scaled_rest_words = {word: self.combined_scaling(
                # Adjust th>
...
# Combine the scaled frequencies
        ...
        if unique_id:
            timestamp = f"{time.time()}{unique_id}"
        else:
            timestamp = int(time.time())
        wc_image_path = os.path.join(
            "/freetxt/website/static/wordcloud", f"wordcloud_{timestamp}.png")
    ...
    def compute_word_frequency(self, tokenized_words, language):
        ...
        if language == 'en':
            ...
            word_freq = word_freq.merge(
                Bnc_corpus[Bnc_corpus['word'].isin(word_freq['word'])], how='left', on='word')
            df = word_freq[['word', 'freq', 'f_Reference']]

        else:
            try:
                column_names = ['word', 'f_Reference']
                corcencc_corpus = pd.read_csv(
                    '/freetxt/website/static/keness/file.raw.pos.sem.wrd.fql', sep='\t', names=column_names)

                # Merge with CorCenCC corpus
                word_freq = word_freq.merge(corcencc_corpus[corcencc_corpus['word'].isin(
                    word_freq['word'])], how='right', on='word')
    ...
    def generate_wordcloud_type(self, input_data, cloud_type, language, cloud_measure, wordlist=None):
        ...
        if cloud_type == 'all_words':
            ...
            n_gram_freq_items = [(' '.join(gram), freq)
                                 for gram, freq in n_gram_freq.items()]
            df_n_grams = pd.DataFrame(
                n_gram_freq_items, columns=['word', 'freq'])
            ...
            df_n_grams = self.calculate_measures(
                df_n_grams, cloud_measure, language)
            ...
            tags_freq.columns = ['Tag', 'Frequency']
            Tags_f_reference = pd.DataFrame()

            # Handle semantic tags based on language
            if language == 'en':
                tags_freq.columns = ['USAS Tags', 'freq']
                Bnc_sementic_tags = pd.read_csv(
                    '/freetxt/website/static/keness/BNC_semantictags.csv')
                merged_df = pd.merge(
                    tags_freq, Bnc_sementic_tags, on='USAS Tags', how='inner')
                merged_df = merged_df.rename(
                    columns={'USAS Tags': 'word', 'f_reference': 'f_Reference'})
                Tags_f_reference = self.calculate_measures(
                    merged_df[['word', 'freq', 'f_Reference']], cloud_measure, language)

            else:
                tags_freq.columns = ['Equivalent Tag', 'freq']
                corcencc_sementic_tags = pd.read_csv(
                    '/freetxt/website/static/keness/Cy_tags.csv')
                merged_df = pd.merge(
                    tags_freq, corcencc_sementic_tags, on='Equivalent Tag', how='inner')
                merged_df = merged_df.rename(
                    columns={'Equivalent_Tag': 'word', 'f_reference': 'f_Reference'})
                Tags_f_reference = self.calculate_measures(
                    merged_df[['word', 'freq', 'f_Reference']], cloud_measure, language)
    ...
    def generate_wordcloud(self, cloud_shape_path, cloud_outline_color, cloud_type, language, cloud_measure, wordlist={}, unique_id=None):
        words_for_cloud, df = self.generate_wordcloud_type(
            self.tokens_with_semantic_tags, cloud_type, language, cloud_measure, wordlist)
